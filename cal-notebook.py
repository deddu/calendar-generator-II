...
def _():
    ...
    _us_tz = zoneinfo.ZoneInfo('America/Chicago')
    _it_tz = zoneinfo.ZoneInfo('Europe/Rome')
    prev_us_dst = _us_tz.dst(datetime(year, 1, 1))
    prev_it_dst = _it_tz.dst(datetime(year, 1, 1))
    ...
    for j in range(0, 13):
        for i in range(0, 32):
            try:
                ...
                curr_us_dst = _us_tz.dst(d)
                curr_it_dst = _it_tz.dst(d)
                ...
                pass
    draw.text((day_w_px * 0.4, day_h_px * 0.6), f'{year}', fill='black', font=f_y, align='left')
    image.save(f'cal-{year}-{dpi}dpi.png')
    image.show()
    return datetime, i, image, it_holidays, j, year, zoneinfo


@app.cell
def _(datetime, i, it_holidays, j, year):
    a=datetime(year, j, i)
    a.strftime('%a')
    a in it_holidays
    return
...
@app.cell
def _(datetime, zoneinfo):
    _us_tz = zoneinfo.ZoneInfo('America/Chicago')
    _it_tz = zoneinfo.ZoneInfo('Europe/Rome')
    _it_tz.dst(datetime(2024, 3, 1))
    return
